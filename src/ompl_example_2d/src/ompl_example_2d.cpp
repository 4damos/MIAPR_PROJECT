...
bool isStateValid(const ob::State *state){
    ...
    //PRZESZKODY  
     if (coordX->values[0]<-2.0&&coordX->values[0]>-5.0){
        if (coordY->values[0]<0.0&&coordY->values[0]>-5.0){
...
nav_msgs::Path Planner2D::planPath(const nav_msgs::OccupancyGrid& globalMap){
    ...
    // DOMYŚLNY auto planner(std::make_shared<og::RRTConnect>(si));
...
void Planner2D::configure(void){
    ...
    //ORG coordXBound->setLow(-1.0);
    //ORG coordXBound->setHigh(13.0);
    coordXBound->setLow(-12.0);
    coordXBound->setHigh(4.0);
    // create bounds for the y axis
    coordYBound.reset(new ob::RealVectorBounds(dim-1));
    // ORG coordYBound->setLow(-5.0);
    // ORG coordYBound->setHigh(5.0);
    coordYBound->setLow(-7.0);
    coordYBound->setHigh(7.0);
